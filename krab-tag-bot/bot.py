...
_LABELS = {
    ...
    "address": "address", "registration address": "address",
    "city": "city", "state": "state", "zip": "zip", "zipcode": "zip",
    "insurance": "insurance_company", "insurance company": "insurance_company",
    "policy": "policy", "policy #": "policy", "policy number": "policy",
    "issued": "issued", "issue date": "issued",
    "expires": "expires", "expiration": "expires", "expiration date": "expires",
}
...
def parse_labeled(text: str) -> dict:
    ...
    for line in (text or "").splitlines():
        ...
        if not m:
            continue
        key = _LABELS.get(m.group(1).strip().lower())
...
def main() -> None:
    ...
    app.add_handler(CommandHandler(["start", "help", "newtag"], cmd_start))
    app.add_handler(CallbackQueryHandler(handle_send_to_group, pattern=r"^send_"))
    app.add_handler(MessageHandler((filters.TEXT | filters.CAPTION) & ~filters.COMMAND, handle_details))
