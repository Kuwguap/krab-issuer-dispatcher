...
def fetch_nj_pdf_preview(payload: Dict[str, Any]) -> NJPdfResult:
    """POST JSON to ``/api/insurance-card-nj`` (no /email) for raw PDF bytes.

    Returns the rendered NJ Temporary Evidence of Insurance PDF without
    triggering email send or portal account creation. Use this when the bot
    wants to preview the card in Telegram before deciding on email delivery.
    """
    try:
    ...
    # /api/insurance-card-nj doesn't take an email field — strip it before sending.
